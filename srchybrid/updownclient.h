...
//	/*FunnyNick*/LPCTSTR	GetUserName(bool bFunnyNick = true) const								{ return (thePrefs.DisplayFunnyNick() && m_pszFunnyNick && bFunnyNick)?m_pszFunnyNick:m_pszUsername; }
	/*FunnyNick*/LPCTSTR	GetUserName(bool bFunnyNick = true) const								{ return (thePrefs.DisplayFunnyNick() && m_pszFunnyNick && bFunnyNick && m_uBanReason==GOOD_BOY)?m_pszFunnyNick:m_pszUsername; }
	// <== Reduce Score for leecher - Stulle
	...
	*/
	void			BanLeecher(LPCTSTR pszReason = NULL, uint8 uReason = 0);
	// <== Reduce Score for leecher - Stulle
...
//--group to aligned int32
	...
	_EBanReason			m_uBanReason; // Reduce Score for leecher - Stulle
...
//	DWORD	dwThisClientIsKnownSince; --> SLAHAM just defined it!
...
// ==> Spread Requests - Stulle
	DWORD	GetSpreadReAskTime() const {return m_dwSpreadReAskTime;}
private:
...
public:
	...
	bool	IsLeecherName(CString Username); // Leecher Name [Xman] - Stulle
	bool	IsSnakeOrGamer(CString Username); // Snake / Gamer Check [Xman] - Stulle
	bool	IsTypicalHex (CString addon); // Hex Check [Xman] - Stulle
...
private:
	...
	DWORD m_ulastNickChage;
	// <== Nick Changer Check [Xman] - Stulle

	// ==> Spread Credits Slot - Stulle
	uint8 m_bSpreadClient;
public:
	uint8 GetSpreadClient() const	{return m_bSpreadClient;}
	void SetSpreadClient(uint8 in){m_bSpreadClient = in;};
	// <== Spread Credits Slot - Stulle

	// ==> Reduce Score for leecher - Stulle
	EBanReason	GetBanReason() const	{ return (EBanReason)m_uBanReason; }
	void SetBanReason(uint8 in)	{m_uBanReason = (EBanReason)in;}
	bool GetScoreReduce() const;
	CString GetBanReasonString() const;
	// <== Reduce Score for leecher - Stulle
...
public:
	...
	bool IsStulleMule() const	{return GetModClient() == MOD_STULLE;}
	// <== Don't count modid fakers - Stulle

	// ==> Prevent from missing an Upload by Xtreme - Stulle
	bool IsXtremeClient() const	{return GetModClient() == MOD_XTREME || GetModClient() == MOD_SCAR;}
	// <== Prevent from missing an Upload by Xtreme - Stulle
};
//#pragma pack()
//>>> eWombat [SNAFU_V3]
